...
@socketio.on("join", namespace="/BINSpy")
def join(message):
    try:
...
@socketio.on("my_room_event", namespace="/BINSpy")
...
@socketio.on("disconnect_request", namespace="/BINSpy")
...
@socketio.on("error", namespace="/BINSpy")
def socket_def_error(d):
    pass

@socketio.on_error("/BINSpy")
def error_handler(e):
    pass

@app.route("/" + BINSpy.home_path)
def home():
    gMaps_free_api_key = 'AIzaSyBUPHAjZl3n8Eza66ka6B78iVyPteC5MgM'
    if (BINSpy.gmaps != ''):
        gMaps_free_api_key = BINSpy.gmaps

    shorten_api = 'AIzaSyCPzcppCT27KTHnxAIQvYhtvB_l8sKGYBs'

    html = BINSpy.injectCSS_Paths(render_template("home.html", async_mode=socketio.async_mode).replace('[OWN_API_KEY_HERE]', gMaps_free_api_key).replace('[LIBS_SRC]', BINSpy.JSFiles[1]['src']).replace('[BINSpy_SRC]', BINSpy.JSFiles[4]['src']))
    return html

if __name__ == 'core.sockets':
    try:
        socketio.run(app, host= '0.0.0.0', port=BINSpy.app_port, debug=False)
    except KeyboardInterrupt:
        socketio.stop()
        BINSpy.validateLicense.terminate()
        sys.exit(0)